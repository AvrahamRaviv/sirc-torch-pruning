"""Autoresearch-style retention-accuracy harness for ConvNeXt-tiny on real ImageNet val.

Pattern (karpathy/autoresearch): one immutable eval harness + one comparable metric +
propose→test→keep/discard→log→repeat. Here the metric is PRE-FT retention top-1 at a matched
MAC target (no fine-tune — recovery FT would be hours/experiment). Each "experiment" = a
channel-importance spec → prune at matched MAC → eval. Calibration + decoded calib batches +
variance/cov stats are computed ONCE and cached in-process; scoring is non-destructive, so
hundreds of specs are cheap.

Touches no Torch-Pruning code — imports the existing helpers/pruner/importance classes only.
New propagation directions live in score_experiments.py (the mutable file).

Usage:
    python -m research.harness --phase baseline           # the ≥10-number leaderboard
    python -m research.harness --phase experiments        # run score_experiments.SPECS
    python -m research.harness --only prop_cov_joincov     # one spec by name
Results append to research/results.jsonl + research/LEADERBOARD.md (skip-if-done by key).
"""
import argparse
import copy
import glob
...
from vbp_common import convnext_tiny, get_val_transform
...
class Ctx:
    def __init__(self, args):
        ...
        self.tf = get_val_transform("convnext", resize=256)
        self.ex = torch.randn(1, 3, 224, 224)
        # dense model (original, un-normalized) for classical scorers
        self.dense = convnext_tiny(pretrained=False)
        ...
        # cached calib batches (decode once) from the LAST shards (reduce eval overlap)
        ...
        # reparam (mean) on a COPY; keep the merged plain net for normnet-score pruning
        self._build_normnet()
        self._var_stats = None        # lazy: {name: (variance, means)}
        self._nci_scores = None
        self._means_by_name = None    # lazy: post-act means for bias compensation
    ...
    def _build_normnet(self):
        ...
        nargs = SimpleNamespace(reparam_variant="mean", norm_bn_momentum=0.01,
                                ...
                                max_batches=self.args.calib_batches, model_type="convnext",
                                exclude_stem=False)
        ...
        # cov/join-cov caches (collected once, reused across cov specs)
    ...
    def base(self):
        """Pruning base = the ORIGINAL dense net. mgr stays ACTIVE (never merged) so every
        propagation spec can re-score; scores map to modules by NAME and the original/merged
        nets share names + channel indices (merge only rescales weights, doesn't restructure),
        so pruning the original at those indices is the equivalent pre-FT base for all specs."""
        return copy.deepcopy(self.dense)

    def var_stats(self, mode):
        """Collect VarianceImportance stats on the ORIGINAL dense, keyed by module name."""
        if self._var_stats is None:
            tl = NM._post_act_target_layers(self.dense, "convnext", self.ex)
    ...
    def means_by_name(self):
        """Post-activation means for bias compensation, keyed by module name (the producer
        whose output feeds the pruned consumer). Adding W[:,c]·μ_c to the consumer bias before
        removing channel c preserves the expected output — the pre-FT analogue of the MLP
        harness's mean-ablation. Collected once on the original dense."""
...
def build_importance_factory(ctx, spec):
    """Return (imp_factory(mdl)->Importance, base_model_cpu, normalizer_for_pruner)."""
    kind = spec["kind"]
    p = spec.get("params", {})
    ...
    if kind == "propagation_iterative":
        ek = ctx.extract_kwargs(cov=p.get("cov", False), join_cov=p.get("join_cov", False),
                                p=p.get("p", 2), relative=p.get("relative", True))
        _ig_ids = {id(m) for m in NM._ignored_layers(ctx.norm_model, "convnext")}
        _ig_names = {n for n, m in ctx.norm_model.named_modules() if id(m) in _ig_ids}
        scores, _ = NM._iterative_propagation_scores(
            ctx.mgr, ctx.ex, ek, model_type="convnext", normalizer=norm,
            drop_per_round=p.get("iter_drop", 128), max_frac=p.get("iter_max_frac", 0.6),
            ignored_names=_ig_names, log=lambda s: None)
        # normalizer folded into greedy ranking → prune with 'none'
        return (lambda mdl: NormalizedNetImportance(mdl, scores, group_reduction="mean",
                normalizer=None), ctx.base(), "none")
    if kind == "custom":
        import score_experiments as SE
        fn = getattr(SE, p["fn"])
        scores = fn(ctx, p)                       # custom direction → scores dict
        # custom decides its own normalizer (often 'none' if it folds shaping in)
...
def run_spec(ctx, spec, limit):
    ...
    ignored = lambda mdl: NM._ignored_layers(mdl, "convnext")
...
def rewrite_leaderboard():
    ...
    with open(LEADERBOARD, "w") as f:
        f.write("# Retention leaderboard — ConvNeXt-tiny, pre-FT top-1 @ matched MAC 2.94G\n\n")
...
def main():
    ...
    ap.add_argument("--confirm_limit", type=int, default=10000,
                    help="re-eval the current best at this larger limit")
    ap.add_argument("--mac_target_g", type=float, default=2.94)
    ...
    ap.add_argument("--no_bias_comp", action="store_true",
                    help="disable bias compensation (mean-ablation). Default ON — adds "
                         "W[:,c]·μ_c to consumer bias on removal, the pre-FT retention booster.")
    args = ap.parse_args()
