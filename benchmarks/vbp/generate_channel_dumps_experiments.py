...
`--save_dir <out_dir>` so run_ddp.py patches it at submit time:
    ...
    python run_ddp.py --out_dir_name CD_rn50_glob_mean            # submit one (docker)
    ... (repeat per folder, or loop the printed list)

Single-process (no torchrun): the bash just runs `python normnet_standalone.py ...` on a
GPU node. No training (epochs 0) — calibrate sigma, score, capture prune mask, dump.

Matrix: {resnet50, convnext_tiny} x {global, local} x {mean, none normalizer}. Each run
dumps all 4 criteria (magnitude, nci, rel, nonrel) -> 4 JSON per folder, 32 total. Point
ExpHandler at BASE_OUT (it globs **/*_channel_scores.json). Scores are config-independent;
only the `kept` mask differs across global/local/normalizer — that is the comparison.
"""
import os
import stat

# --- remote paths (cluster) --- (override BASE_OUT via env for local dry-runs)
BASE_OUT = os.environ.get("CD_BASE_OUT", "/algo/NetOptimization/outputs/NORMNET/ResNet50")
...
def main():
    ...
    for model in MODELS:
        ...
        for is_global in GLOBALS:
            for norm in NORMALIZERS:
                ...
                # out_dir placeholder; run_ddp.py re-patches --save_dir (kept last) at submit.
    ...
    print(f"\n{len(made)} dump experiments (ratio={RATIO}, calib={CALIB}). Submit each:")
    for tag in made:
